...
async def generate_document(request: DocumentRequest) -> GeneratedEmail:
    """
    ...
    if request.doc_type == "offer_letter":
        html_content = await _generate_offer_email(request)
    elif request.doc_type == "rejection_letter":
        html_content = await _generate_rejection_email(request)
    elif request.doc_type == "policy_update":
        html_content = await _generate_policy_update_email(request)
    else:
...
async def _generate_offer_email(data: OfferLetterData) -> str:
...
async def _generate_rejection_email(data: RejectionLetterData) -> str:
...
async def _generate_policy_update_email(data: PolicyUpdateData) -> str:
...
if __name__ == '__main__':
    async def main():
        if not Config.GOOGLE_API_KEY:
            ...
            offer_email = await generate_document(offer_data)
            ...
            rejection_email = await generate_document(rejection_data)
    ...
    """,
            ...
            policy_email = await generate_document(policy_data)
