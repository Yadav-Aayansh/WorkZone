...
async def run_interview_test():
    ...
    try:
        ...
        print("✓ Interview Started (Session saved to Redis)")
        print("Session ID:", start_res.session_id)
        print("First Question:", start_res.question_text.question)  # Changed from first_question
        print("Question Type:", start_res.question_text.type)  # Changed from first_question
        print("Focus Area:", start_res.question_text.focus_area)  # Changed from first_question
        print("First Question Audio URL:", start_res.question_audio_url)  # Changed from first_question_audio_url
    ...
    # First question from start response
    print(f"\nQ{current_index + 1}: {start_res.question_text.question}")  # Changed from first_question
    print(f"   Type: {start_res.question_text.type} | Focus: {start_res.question_text.focus_area}")  # Changed from first_question
    ...
    while True:
        ...
        try:
            ...
            # Check if it's a clarification
            if res.status == "clarification_needed":  # Changed from is_clarification
                print("💡 CLARIFICATION PROVIDED (not counted as a question)")
                print(f"   {res.clarification}")
                print(f"\nPlease answer the same question:")
                print(f"Q{res.question_index + 1}: {res.question_text.question}")  # Changed from next_question
                continue  # Ask for answer again without incrementing
        ...
        if res.status == "completed":
            ...
            if res.clarification:
                ...
                print(res.clarification)
                print("="*70)
                if res.question_audio_url:  # Changed from next_question_audio_url
                    print(f"\nClosing Message Audio: {res.question_audio_url}")
        ...
        # Display next dynamically generated question
        current_index = res.question_index  # Changed from next_question_index
        
        # Show poor answer warning if applicable
        if res.poor_answer_count and res.poor_answer_count > 0:
            print(f"\n⚠ Warning: {res.poor_answer_count} poor answer(s) detected. Interview may end early after 3.")
        
        print(f"\nQ{current_index + 1}: {res.question_text.question}")  # Changed from next_question
        print(f"   Type: {res.question_text.type} | Focus: {res.question_text.focus_area}")  # Changed from next_question
        print(f"   Total Questions So Far: {res.total_questions_asked}")
