...
# NOT: If the candidate is asking for clarification, that is NOT a poor answer.
...
# async def is_clarifying_question(answer: str, question: str) -> bool:
#     """
#     Detect ONLY when candidate explicitly asks a cross-question for clarification.
    
#     Must be:
#     1. Very short (< 15 words)
#     2. Has question indicator (? or question word)
#     3. Directly asking about the question itself (not just a vague answer)
    
#     This is STRICT - only catches obvious clarification requests, not incomplete answers.
#     """
#     answer_lower = answer.lower().strip()
#     word_count = len(answer.split())
    
#     # Must be short (cross-questions are brief)
#     if word_count > 15:
#         return False
...
#     has_question_indicator = '?' in answer or any(
#         word in answer_lower 
#         for word in ['which', 'what do you mean', 'could you clarify', 'can you elaborate', 'specifically']
#     )
    
#     if not has_question_indicator:
#         return False
    
#     # Strict patterns - only obvious cross-questions
#     explicit_clarification_patterns = [
#         r'^which\s+(one|ones|service|services|project|choice|choices|tool|tools)',
#         r'^what\s+do\s+you\s+mean',
#         r'^could\s+you\s+clarify',
#         r'^can\s+you\s+elaborate',
#         r'^can\s+you\s+explain',
#         r'^what\s+specifically',
#         r'^which\s+specifically',
#         r'you\s+mean\s+\w+\?',
#         r'^clarify\s+please',
#         r'^could\s+you\s+be\s+more\s+specific',
#     ]
    
#     # Check if matches explicit clarification patterns
#     for pattern in explicit_clarification_patterns:
#         if re.search(pattern, answer_lower):
#             return True  # Definite clarification question
    
#     # If it's very short (< 5 words) with '?' at the end, likely a clarification
#     if word_count < 5 and '?' in answer:
#         # But not if it's just "what?" or "why?" alone (dismissive)
#         if word_count > 1:
#             return True
...
# async def is_poor_answer(answer: str, question: str = "") -> bool:
#     """
#     Use LLM to intelligently detect poor answers based on semantic understanding.
    
#     Updated to be more lenient:
#     - Accepts 2-3 line answers as reasonable
#     - Only flags truly poor responses (refusals, "I don't know", complete off-topic)
#     - Clarifying questions are NOT considered poor answers
#     """
#     # First check if it's a clarifying question
#     if question and await is_clarifying_question(answer, question):
#         return False  # Clarifying questions are NOT poor answers
    
#     # More lenient word count threshold
#     answer_stripped = answer.strip()
#     word_count = len(answer_stripped.split())
    
#     # Only very short answers (< 3 words) are automatically poor
#     if word_count < 3:
#         return True
    
#     # Use LLM for semantic understanding with updated criteria
#     try:
#         prompt = f"""Analyze if this interview answer is poor quality.
...
# - Provides relevant information (even 2-3 lines is fine)
# - Shows some knowledge or experience
# - Addresses the question asked
# - Has reasonable substance (doesn't need to be lengthy)
# - Short but on-topic answers are GOOD

# IMPORTANT: Be LENIENT. A 2-3 line relevant answer is GOOD, not poor.
# Only mark as POOR if truly refusing, dismissive, or completely off-topic.
...
#             {
#                 "role": "system",
#                 "content": "You are a LENIENT interviewer evaluating answers. Short but relevant answers (2-3 lines) are GOOD. Only mark truly bad responses as POOR. Respond with only 'POOR' or 'GOOD'."
#             },
#             {"role": "user", "content": prompt}
#         ]
